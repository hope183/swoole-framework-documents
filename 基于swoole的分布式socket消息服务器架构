...
&#160; &#160; &#160; &#160;2.`Socket1`接收到消息，根据`key`从Redis取出`Client2`的连接信息，连接在本机，直接推送给`Client2`，流程结束。
&#160; &#160; &#160; &#160;3.如果连接不在本机，把消息推送到`转发服务器`,由`转发服务器`把该消息推送给连接所在`消息服务器`，消息服务器接收消息，推送给`Client2`。
&#160; &#160; &#160; &#160;4.消息发送结束。
##编码实现
* **Socket**
    在`socket1`上创建一个`server.php`,内容如下：      
      <?php
        //服务端
        ...
        $serv->start();
    在`socket2`上只需把ip变更一下即可。`192.168.0.201`变更为`192.168.0.202`.
* **Proxy**
    在转发服务器上建立脚本`proxy.php`，内容如下：
...
##测试
&#160;&#160;&#160;&#160;**注意**`开启顺序`
&#160;&#160;&#160;&#160;1.开启转发服务器`php proxy.php`
&#160;&#160;&#160;&#160;2.分别开启socket服务器`php server.php`
&#160;&#160;&#160;&#160;可以在转发服务器上看到两个消息服务器已经连接
&#160;&#160;&#160;&#160;![Alt text](./socket-accept.jpg)
&#160;&#160;&#160;&#160;3.开始测试，分别打开两个`telnet`,连接两个消息服务器，发送消息测试：
&#160;&#160;&#160;&#160;**登陆**
&#160;&#160;&#160;&#160;![Alt text](./socket-login-201.jpg)
&#160;&#160;&#160;&#160;**发送消息测试**
&#160;&#160;&#160;&#160;![Alt text](./socket-msg-recv.jpg)
&#160;&#160;&#160;&#160;消息成功接收。
